class Collective::LinkedList {
    ...
    proto method new(+values, Bool() :$reversed --> ::?CLASS:D) {*}
    multi method new() {
        my $self := self.CREATE;
        $self!SET-SELF(IterationEnd, $self);
    }
    multi method new(Iterable \values is readonly, :$reversed) {
    ...
    multi method new(**@values is raw, :$reversed) {
        self.new(@values, :$reversed);
    }
...
Defined as:
    ...
    proto method new(+values, Bool() :$reversed
                 --> Collective::LinkedList:D)

Creates and returns a new C<Collective::LinkedList> with the provided
values. Use the C<:reversed> adverb to store values in reverse order.
